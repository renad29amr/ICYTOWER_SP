...
tex_widerBlocks, tex_welcome, tex_bomb, tex_boom, tex_hurryUp, tex_cracks;

// Sprites
Sprite background, background2, background3, ground, wall, wall2, interface, hand, pause_menu, head, gameover, enterName, winner, newstage,
highscoreMenu, barrier, barrier2, ice[max_lives], clock1, clockhand, instructions, star, credits, sweet, thankyou, welcome, bomb, boom, hurryUp, cracks;
...
struct Players
{
    ...
    void playerRotation(float deltatime,vector<BLOCKS>& blocksList, int level, bool& viewpaused, Clock& viewtimer)
    {
...
void generationBlocks(vector<BLOCKS>& blocksList, Players& player, int level)
{
    ...
    int numBlocks = (level == 1) ? 5 : (level == 2) ? 5 : 7;
                ...
                if (lastBlockIndex != -2 && currentBlockIndex != lastBlockIndex && lastBlockIndex < static_cast<int>(blocksList.size()))
                {
                    ...
                    for (int j = startIndex + 1; j < endIndex; j++)
                    {
                        if (!blocksList[j].isIntersected)
                        {
                            ...
                            score += (blocksList[j].level == 1) ? 20 : (block.level == 2) ? 25 : 30;
                            floors++;
...
int main()
{
    ...
    if (start_game)
    {
        ...
        while (window.isOpen())
        {
            ...
            player.playerRotation(deltatime, blocksList, currentLevel, viewpaused, viewtimer);
