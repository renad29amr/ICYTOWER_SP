...
void generationBlocks(vector<BLOCKS>& blockslist, vector<bool>& blockIntersected, Players& player, int level)
{
    float y;
    ...
    {
        blockslist.clear();
        blockIntersected.clear(); // Clear blockIntersected to match blockslist
        y = HEIGHT - 200;
    }
    else
    {
    ...
    // Resize blockIntersected to match blockslist size
    blockIntersected.resize(blockslist.size(), false);
}
...
// Collision function
void collision(vector<BLOCKS>& blockslist, vector<bool>& blockIntersected, Players& player, Font font, Text& Score, Text& text_start, Text& text_exit, Text& text_play_again, Text& text_highscore, Text& text_sound)
{


    if (blockIntersected.size() != blockslist.size())
    {
        blockIntersected.resize(blockslist.size(), false);
    }
    ...
    for (size_t i = 0; i < blockslist.size(); ++i)
    {
        ...
        if (player.sprite.getGlobalBounds().intersects(block.blocksSprite.getGlobalBounds()))
        {
            if (player.velocity_y > 0 && (player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= block.blocksSprite.getPosition().y))
            {
                ...
                if (!blockIntersected[i])
                {
                    blockIntersected[i] = true;
                    if (currentLevel == 1)
                    {
                        score += 10;
                    }
                ...
                // Only calculate skipped blocks if this is not the first block landed on
    ...
    if (barrierSpawned && player.sprite.getGlobalBounds().intersects(levelBarrier.getGlobalBounds()))
    {
        if (player.velocity_y > 0 && (player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= levelBarrier.getPosition().y))
        {
            ...
            if (!level2Generated)
            {
                sound_cheer.play();
                currentLevel = 2;
                generationBlocks(blockslist, blockIntersected, player, currentLevel);
...
void reset(Players& player, vector<BLOCKS>& blocksList, vector<bool>& blockIntersected, int& score, int& floors, Text& Score)
{
    // Reset game state variables
    score = 0;
    floors = 0;
    lives = 3; // Assuming max_lives is 3 as per global constant
    currentLevel = 1;
    ...
    clockFill = 0.0f;
    lastBlockIndex = -1;
    currentBlock = nullptr;  // if accessible here, or reset in collision logic
    ...
    player.sprite.setPosition(500, 650); // Initial player position
    player.sprite.setScale(2.4f, 2.4f);  // Reset scale to default
    player.sprite.setTextureRect(IntRect(0, 0, 38, 71)); // Reset to initial frame
    lastSafePosition = player.sprite.getPosition(); // Update last safe position

    // Reset UI elements
    Score.setString("Score: " + to_string(score));
    Score.setFillColor(Color::White);
    Score.setPosition(50, 900); // Reset score position
    ...
    view.setSize(1000, 1000); // Ensure view size matches window
    ...
    /*if (background_music.getStatus() != Music::Playing) {
...
bool startMenu(Sprite hand, Sprite interface, Sprite enterName, Font font, Text text_start, Text text_sound, Text text_highscore, Text text_exit, Texture& tex_heads, Sprite head, Texture& tex_pauseMenu, Texture& tex_highscore, Sprite& highscore)
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (event.type == Event::KeyPressed && !isPressed && !showHighscore)
                {
                    isPressed = true;
                    if (event.key.code == Keyboard::Up)
                    {
...
bool pauseMenu(Players& player, Sprite interface, Sprite hand, Font font, Text text_sound, Text text_exit, Text text_start, vector<BLOCKS>& blocksList, vector <bool>& blockIntersected, Text text_play_again, Text& Score)
{
    int menuSelection = 0;
    bool isPressed = false;
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::KeyPressed && !isPressed)
            {
                ...
                if (event.key.code == Keyboard::Enter)
                {
                    ...
                    {
                        reset(player, blocksList, blockIntersected, score, floors, Score);
                        return true;
                    }
...
bool gameOver(Players& player, Sprite hand, Texture& tex_gameover, Texture& tex_pauseMenu, Font& font, Text text_play_again, Text text_exit, vector<BLOCKS>& blocksList, vector <bool>& blockIntersected, Text& Score, Text& timerText)
{
    int menuSelection = 0;
    bool isPressed = false;
...
bool winMenu(RenderWindow& window, Players& player, Sprite hand, Texture& tex_gameover, Texture& tex_pauseMenu, Font& font, Text text_play_again, Text text_exit, vector<BLOCKS>& blocksList, vector<bool>& blockIntersected, Text& Score, Text& timerText)
{
    sound_cheer.play();
...
int main()
{
    ...
    if (start_game)
    {
        ...
        vector<bool> blockIntersected; // Track which blocks have been intersected
        player.sprite.setOrigin(player.sprite.getLocalBounds().width / 2, 0);

        random_device rd;
        srand(rd());
        generationBlocks(blocksList, blockIntersected, player, currentLevel); // Pass blockIntersected
        ...
        while (window.isOpen())
        {
            ...
            {
                bool resumeGame = pauseMenu(player, interface, hand, font, text_sound, text_exit, text_start, blocksList, blockIntersected, text_play_again, Score);
                if (!resumeGame)
                {
            ...
            if (player.sprite.getPosition().y > viewBottom)
            {
                ...
                else
                {
                    ...
                    sound_gameover.play();
                    bool resumeGame = gameOver(player, hand, tex_gameover, tex_pauseMenu, font, text_play_again, text_exit, blocksList, blockIntersected, Score, timerText);
            ...
            collision(blocksList, blockIntersected, player, font, Score, text_start, text_exit, text_play_again, text_highscore, text_sound);
            ...
            if (currentLevel == max_levels && isGround)
            {
                    ...
                    player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= blocksList.back().blocksSprite.getPosition().y)
                {
                    ...
                    bool resumeGame = winMenu(window, player, hand, tex_gameover, tex_pauseMenu, font, text_play_again, text_exit, blocksList, blockIntersected, Score, timerText);
                    if (!resumeGame)
                    {
                        startMenu(hand, interface, enterName, font, text_start, text_sound, text_highscore, text_exit, tex_heads, head, tex_pauseMenu, tex_highscore, highscore);
                    }
