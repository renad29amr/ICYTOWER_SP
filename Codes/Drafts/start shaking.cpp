...
void vibrateAndDisappearBlocks(Players& player, vector<BLOCKS>& blocksList, float deltaTime)
{
    if (currentLevel == 3)
    {
        static float shakeTimer = 0.0f; // Persistent timer for shake effect
        shakeTimer += deltaTime; // Increment timer based on frame time

        float shakeAmplitude = 4.0f; // Pixels to shake
        float shakeFrequency = 50.0f; // Shake speed
        float shakeOffset = shakeAmplitude * sin(shakeTimer * shakeFrequency);

        bool playerOnLevel3Block = false;
        size_t intersectedIndex = 0;

        // Check if player is intersecting any Level 3 block
        for (size_t i = 0; i < blocksList.size(); ++i)
        {
            if (blocksList[i].level == 3 && player.sprite.getGlobalBounds().intersects(blocksList[i].blocksSprite.getGlobalBounds()))
            {
                if ((player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= blocksList[i].blocksSprite.getPosition().y))
                {
                    playerOnLevel3Block = true;
                    intersectedIndex = i;
                    break;
                }
            }
        ...
        // Apply shake effect to intersected Level 3 block and all Level 3 blocks below it
        if (playerOnLevel3Block)
        {
            for (size_t i = 0; i < blocksList.size(); ++i)
            {
                if (blocksList[i].level == 3 && blocksList[i].blocksSprite.getPosition().y >= blocksList[intersectedIndex].blocksSprite.getPosition().y)
                {
                    float currentX = blocksList[i].blocksSprite.getPosition().x;
                    blocksList[i].blocksSprite.setPosition(currentX + shakeOffset, blocksList[i].blocksSprite.getPosition().y);
                }
            }
        }
    }
...
bool winMenu(Players& player, Font font, Text text_play_again, Text text_exit, vector<BLOCKS>& blocksList, Text& Score, Text& timerText, vector<Feature>& featuresList, bool& viewpaused, bool& win, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    sound_winner.play();
    sound_cheer.play();
    int menuSelection = 0;
...
void drawTransitionEffect(Font& font, Vector2f camPos, int currentLevel, float deltaTime, Clock& transitionClock, Sprite& newstage, Barrier* barrier)
{
...
void draw(Players& player, vector<BLOCKS>& blocksList, Text& Score, Font font, Text& timerText, Text& text_skipped, float& deltatime, Clock& rotation, Clock& Timer, RectangleShape& OclockFill, bool& showSuperJump, vector<Feature>& featuresList)
{
    ...
    if (currentLevel >= 2 && currentLevel <= 3)
    {
        ...
        drawTransitionEffect(font, camPos, currentLevel, deltatime, transitionClock, newstage, barrier);
    }
    ...
    if (start_game)
    {
        ...
        while (window.isOpen())
        {
            ...
            if (!viewpaused)
            {
                ...
                vibrateAndDisappearBlocks(player, blocksList, deltatime);
            }
                ...
                for (auto it = blocksList.rbegin(); it != blocksList.rend(); ++it)
                {
                    if (it->level == 3)
                    {
                            ...
                            player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= it->blocksSprite.getPosition().y)
                        {
                            ...
                            bool resumeGame = winMenu(player, font, text_play_again, text_exit, blocksList, Score, timerText, featuresList,viewpaused,win,viewtimer,Timer,OclockFill);
                            if (!resumeGame)
                            {
                                reset(player, blocksList, Score, viewpaused, win, rotation, viewtimer, Timer, OclockFill, featuresList);
                                startMenu(font, text_start, text_sound, text_highscore, text_exit, Timer);
