...
tex_enterName, tex_winner, tex_newstage, tex_highscore, tex_block2, tex_background2, tex_barrier, tex_ice, tex_clock, tex_clockhand, tex_hurryUp;

// Sprites
Sprite background, ground, wall, wall2, interface, hand, pause_menu, head, gameover, enterName, winner, newstage, highscore, levelBarrier, ice[max_lives], clock1, clockhand, hurryUp;
...
struct BLOCKS
{
    ...
    bool isIntersected;  // Track if block has been intersected
    ...
    BLOCKS(Texture& texblocks, float xposition, float yposition, int blockLevel)
    {
        ...
        isIntersected = false; // Initialize as not intersected
...
struct Players
{
    ...
    Vector2f rotationCenter; // Center point for screen-wide rotation
    float rotationRadius; // Radius for circular rotation path
    bool isRotating; // Tracks if screen-wide rotation is in progress
    float rotationTimer; // Tracks time since rotation started
        ...
        if (wallJumping) {
            ...
            if (t < 0.5f && fastViewActive) {
                ...
                if (!viewpaused && sprite.getPosition().y < HEIGHT / 3) {
                    if (currentLevel == 1) {
                        view.move(0, NORMAL_VIEW_SPEED * deltaTime);
                    }
                }
            }
...
void generationBlocks(vector<BLOCKS>& blockslist, Players& player, int level)
{
    ...
    nonIntersectedBlocks = 0; // All blocks are non-intersected initially
}
    ...
    for (size_t i = 0; i < blockslist.size(); ++i)
    {
        ...
        if (player.sprite.getGlobalBounds().intersects(block.blocksSprite.getGlobalBounds()))
        {
            if (player.velocity_y > 0 && (player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= block.blocksSprite.getPosition().y))
            {
                ...
                currentBlock = &block; // Fixed: Assign address of block
                currentBlockIndex = i;
                landed = true;
                ...
                if (!block.isIntersected)
                {
                    ...
                    //nonIntersectedBlocks--; // Decrement non-intersected count
                ...
                if (lastBlockIndex != -1 && currentBlockIndex != lastBlockIndex && lastBlockIndex < static_cast<int>(blockslist.size()))
                {
                    ...
                    for (int j = startIndex + 1; j < endIndex; j++)
                    {
                        if (!blockslist[j].isIntersected)
                        {
                            ...
                            nonIntersectedBlocks++; // Decrement for each skipped block
...
void initializeObject()
{
    ...
    if (!tex_hurryUp.loadFromFile("hurryUp.png")) return;
    ...
    hurryUp.setScale(1.5, 1.5);
    hurryUp.setPosition(500, 400);
...
void reset(Players& player, vector<BLOCKS>& blocksList, int& score, int& floors, Text& Score)
{
    ...
    lastBlockIndex = -1;
    currentBlock = nullptr;
    nonIntersectedBlocks = 0; // Reset non-intersected blocks count
...
void draw(Players& player, vector<BLOCKS>& blocksList, Text& Score, Font& font, Text& timerText, float& deltatime, int& floorsSkipped)
{
    window.setView(view);
...
int main()
{
    ...
    if (start_game)
    {
        ...
        vector<bool> blockIntersected;
        int floorsSkipped;// Track which blocks have been intersected
        player.sprite.setOrigin(player.sprite.getLocalBounds().width / 2, 0);

        random_device rd;
        srand(rd());
        generationBlocks(blocksList, player, currentLevel); // Pass blockIntersected
        ...
        while (window.isOpen())
        {
            ...
            window.clear();
            draw(player, blocksList, Score, font, timerText, deltatime, floorsSkipped);
            window.display();
        }
    }
    return 0;
