...
const int WIDTH = 1920, HEIGHT = 1080; // Updated to 1920x1080
...
// Sprites
Sprite background, background2, ground, wall, wall2, interface, hand, pause_menu, head, gameover, enterName, winner, newstage, highscore, levelBarrier, ice[max_lives], clock1, clockhand, instructions, star;
...
void adjustViewAspectRatio(View& view, RenderWindow& window, float desiredAspectRatio)
{
    ...
    {
        // Window is wider than 16:9, add letterboxes on sides
    ...
    {
        // Window is taller than 16:9, add letterboxes on top/bottom
...
struct Players
{
    ...
    Players()
    {
        ...
        sprite.setPosition(960, 650); // Adjusted for 1920x1080
...
void initializeObject()
{
    ...
    enterName.setPosition(1050, 650);
    ...
    interface.setScale(static_cast<float>(1920) / tex_interface.getSize().x, // Updated for 1920x1080
        static_cast<float>(1080) / tex_interface.getSize().y);
    interface.setPosition(0, 0);

    instructions.setTexture(tex_instructions);
    instructions.setScale(static_cast<float>(1920) / tex_instructions.getSize().x, // Updated for 1920x1080
        static_cast<float>(1080) / tex_instructions.getSize().y);
    instructions.setPosition(0, 0);

    background.setTexture(tex_background);
    background.setScale(static_cast<float>(1920) / tex_background.getSize().x, // Updated for 1920x1080
        static_cast<float>(1080) / tex_background.getSize().y);

    background2.setTexture(tex_background2);
    background2.setScale(static_cast<float>(1920) / tex_background2.getSize().x, // Updated for 1920x1080
        static_cast<float>(1080) / tex_background2.getSize().y);
    ...
    wall2.setPosition(1760, 0); // Adjusted for 1920x1080 (880 * 2)
    wall2.setScale(1.5, 3);
    Xright = wall2.getGlobalBounds().width + 810; // Adjusted for 1920x1080
...
void heads()
{
    ...
    head.setPosition(1520, 500); // Adjusted for 1920x1080 (800 * 1.9)
...
void reset(Players& player, vector<BLOCKS>& blocksList, Text& Score, bool& viewpaused, bool& win, Clock& rotation, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    player.sprite.setPosition(960, 750); // Adjusted for 1920x1080
    ...
    view.setCenter(Vector2f(960, 540)); // Center for 1920x1080
    view.setSize(1920, 1080); // Set view size to 1920x1080
    adjustViewAspectRatio(view, window, 1920.0f / 1080.0f); // Enforce 16:9 aspect ratio
    window.setView(view);

    ground.setPosition(0, 900);
    wall.setPosition(0, 0);
    wall2.setPosition(1760, 0); // Adjusted for 1920x1080
...
bool startMenu(Font font, Text text_start, Text text_sound, Text text_highscore, Text text_exit, Clock& Timer)
{
    ...
    hand.setPosition(1000, 670); // Adjusted for 1920x1080
    ...
    displayName.setPosition(1200, 905);
    ...
    while (window.isOpen())
    {
            ...
            else
            {
                if (event.type == Event::KeyPressed && !isPressed && !showHighscore && !showInstructions)
                {
                    ...
                    if (event.key.code == Keyboard::Up)
                    {
                        ...
                        hand.setPosition(1000, 670 + 60 * menuSelection); // Adjusted for 1920x1080
                    }
                    ...
                    if (event.key.code == Keyboard::Down)
                    {
                        ...
                        hand.setPosition(1000, 660 + 60 * menuSelection); // Adjusted for 1920x1080
                    }
                    ...
                    if (event.key.code == Keyboard::Enter)
                    {
                        ...
                        if (menuSelection == 1)
                        {
                            ...
                            soundOptions(font);
                            hand.setPosition(960, 610 + 60 * menuSelection); // Adjusted for 1920x1080
                        }
        ...
        if (showStartMenu)
        {
            ...
            text_sound.setPosition(1082, 725); // Adjusted for 1920x1080 (571 * 1.9)
            ...
            text_highscore.setPosition(1082, 780); // Adjusted for 1920x1080
            ...
            text_credit.setPosition(1082, 835); // Adjusted for 1920x1080
            ...
            text_instructions.setPosition(1082, 890); // Adjusted for 1920x1080
            ...
            text_exit.setPosition(1082, 950); // Adjusted for 1920x1080
...
bool pauseMenu(Players& player, Font font, Text text_sound, Text text_exit, Text text_start, vector<BLOCKS>& blocksList, Text text_play_again, Text& Score, bool& viewpaused, bool& win, Clock& rotation, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::KeyPressed && !isPressed)
            {
                ...
                if (event.key.code == Keyboard::Up)
                {
                    ...
                    hand.setPosition(561.6, 415 + 64.8 * menuSelection);
                }
        ...
        text_play_again.setPosition(648, 486);
        ...
        text_sound.setPosition(648, 562);
...
bool gameOver(Players& player, Font font, Text text_play_again, Text text_exit, vector<BLOCKS>& blocksList, Text& Score, Text& timerText, bool& viewpaused, bool win, Clock& rotation, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    hand.setPosition(620, 560);
    pause_menu.setPosition(600, 300);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::KeyPressed && !isPressed)
            {
                ...
                if (event.key.code == Keyboard::Up)
                {
                    ...
                    hand.setPosition(620, 560 + 70 * menuSelection);
                }
        ...
        Score.setPosition(720, 350);

        Floor.setFillColor(Color::Black);
        Floor.setPosition(720, 420);

        timerText.setFillColor(Color::Black);
        timerText.setPosition(720, 490);
        ...
        text_play_again.setPosition(720, 560);
...
void draw(Players& player, vector<BLOCKS>& blocksList, Text& Score, Font font, Text& timerText, Text& text_skipped, float& deltatime, Clock& rotation, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    wall2.setPosition(1760 + camPos.x - WIDTH / 2 + 40, camPos.y - HEIGHT / 2); // Adjusted for 1920x1080
    ...
    levelText.setPosition(camPos.x - WIDTH / 2 + 1280, camPos.y - HEIGHT / 2); // Adjusted for 1920x1080 (680 * 1.9)

    text_skipped.setString(to_string(nonIntersectedBlocks));

    background.setPosition(camPos.x - WIDTH / 2, camPos.y - HEIGHT / 2);

    background2.setPosition(camPos.x - WIDTH / 2, camPos.y - HEIGHT / 2);
    ...
    if (doneLevel1)
    {
        ...
        {
            sound_cheer.play();
            newstage.setPosition(0, levelBarrier.getPosition().y - 500 + (transitionTime / transitionDuration) * 600.0f);
...
// Updated function to maintain 16:9 aspect ratio
...
int main()
{
    ...
    text_start.setPosition(1082, 670); // Adjusted for 1920x1080 (571 * 1.9)
    ...
    if (start_game)
    {
        ...
        view.setSize(1920, 1080); // Set view size to 1920x1080
        view.setCenter(Vector2f(960, 540)); // Center at (960, 540) for 1920x1080
        adjustViewAspectRatio(view, window, 1920.0f / 1080.0f); // Enforce 16:9 aspect ratio
        window.setView(view);
        ...
        while (window.isOpen())
        {
            ...
            if (player.sprite.getPosition().y > viewBottom)
            {
                ...
                if (lives > 1)
                {
                    ...
                    player.sprite.setTextureRect(IntRect(0, 0, 38, 71));
                    player.onBarrier = true;
                    view.setCenter(Vector2f(960, lastSafePosition.y + 100)); // Adjusted for 1920x1080
                    adjustViewAspectRatio(view, window, 1920.0f / 1080.0f);
                    window.setView(view);
                    viewpaused = true;
                    viewtimer.restart();
