...
static float speedUpX = 0.0f, speedUpY = 0.0f;
...
tex_winner, tex_newstage, tex_highscore, tex_block2, tex_background2, tex_barrier, tex_ice, tex_clock, tex_clockhand, tex_instructions, tex_star, tex_credits, tex_sweet, tex_speedUp;

// Sprites
Sprite background, background2, ground, wall, wall2, interface, hand, pause_menu, head, gameover, enterName, winner, newstage, highscore, levelBarrier, ice[max_lives], clock1, clockhand, instructions, star, credits, sweet, speedUp;

// Sounds
SoundBuffer buffer_menu_choose, buffer_menu_change, buffer_jump, buffer_sound_gameover, buffer_sound_cheer, buffer_sound_rotate, buffer_sound_gonna_fall, buffer_sound_sweet;
...
void collisionANDlevelTransition(vector<BLOCKS>& blocksList, Players& player, Text& Score, Text& text_skipped, bool& showSpeedUp)
{
...
void initializeObject()
{
    ...
    tex_speedUp.loadFromFile("speedup.png");
    ...
    speedUp.setTexture(tex_speedUp);
    speedUp.setScale(1, 1);
                ...
                if (event.type == Event::KeyPressed && !isPressed && !showHighscore && !showInstructions && !showCredits)
                {
                    ...
                    if (event.key.code == Keyboard::Enter)
                    {
                        ...
                        if (menuSelection == 1)
                        {
                            ...
                            soundOptions(font);
                            hand.setPosition(960, 610 + 60 * menuSelection);
                        }
...
bool pauseMenu(Players& player, Font font, Text text_sound, Text text_exit, Text text_start, vector<BLOCKS>& blocksList, Text text_play_again, Text& Score, bool& viewpaused, bool& win, Clock& rotation, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::KeyPressed && !isPressed)
            {
                ...
                if (event.key.code == Keyboard::Up)
                {
                    ...
                    hand.setPosition(760, 320 + 100 * menuSelection);
                }
        ...
        text_play_again.setPosition(850, 410);
        ...
        text_sound.setPosition(850, 510);
...
void draw(Players& player, vector<BLOCKS>& blocksList, Text& Score, Font font, Text& timerText, Text& text_skipped, float& deltatime, Clock& rotation, Clock& Timer, RectangleShape& OclockFill,bool & showSpeedUp)
{
    window.setView(view);
    ...
    {
        showSpeedUp = false;
        speedUpX = camPos.x - WIDTH / 2 + (rand() % (Xright - Xleft)) + Xleft;
        speedUpY = camPos.y - HEIGHT / 2 + (rand() % (HEIGHT - 200)) + 100;
        speedUp.setPosition(speedUpX, speedUpY);
        window.draw(speedUp);
...
int main()
{
    ...
    bool viewpaused = false, win = false,showSpeedUp=false;
    ...
    if (start_game)
    {
        ...
        while (window.isOpen())
        {
            ...
            collisionANDlevelTransition(blocksList, player, Score, text_skipped,showSpeedUp);
            ...
            window.clear();
            draw(player, blocksList, Score, font, timerText, text_skipped, deltatime, rotation, Timer, OclockFill,showSpeedUp);
            window.display();
        }
    }
    return 0;
