...
barrier, barrier2, ice[max_lives], clock1, clockhand, instructions, star, credits, sweet, thankyou, welcome,bomb ,boom;
...
struct BLOCKS
{
    ...
    BLOCKS(Texture& texblocks, float xposition, float yposition, int level)
    {
        ...
        originalXscale = (rand() % 2) + (level == 1 ? 2.25 : level == 2 ? 2 : 1.75);
...
void view_movement(float& deltatime)
{
    ...
    else if (currentLevel == 2)
    {
        ...
        view.move(0, VIEW_SPEED_3* deltatime);
}
...
struct Players
{
    ...
    void playerRotation(float deltatime, View& view, vector<BLOCKS>& blocksList, int level, bool& viewpaused, Clock& viewtimer)
    {
...
void generationFeatures(Players& player, Text& Score, vector<Feature>& featuresList, int level )
{
    ...
    if (featureTimer.getElapsedTime().asSeconds() >= spawnInterval)
    {
        ...
        if ((rand() % 100) / 100.0f < spawnProbability)
        {
            ...
            int featureType = rand() % 5; // 0: superJump, 1: extraLife, 2: loseLife , 3: widerBlocks  , 4:bomb
            string type;
            Texture* texture;
    ...
    if (start_game)
    {
        ...
        while (window.isOpen())
        {
            ...
            if (!viewpaused && player.sprite.getPosition().y < HEIGHT / 7)
            {
                ...
                adjustViewAspectRatio(1920.0f / 1080.0f);
                generationFeatures(player, Score, featuresList,currentLevel);
                window.setView(view);
            }
            ...
            player.playerRotation(deltatime, view, blocksList, currentLevel, viewpaused, viewtimer);
