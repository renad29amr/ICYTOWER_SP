...
tex_winner, tex_newstage, tex_highscore, tex_block2, tex_background2, tex_barrier, tex_ice, tex_clock, tex_clockhand, tex_instructions, tex_star, tex_credits, tex_sweet,
tex_thankyou, tex_superJump, tex_extraLife, tex_loseLife, tex_widerBlocks, tex_welcome;
...
ice[max_lives], clock1, clockhand, instructions, star, credits, sweet, thankyou, welcome;
...
struct BLOCKS
{
    ...
    BLOCKS(Texture& texblocks, float xposition, float yposition, int level)
    {
        ...
        blocksSprite.setOrigin(0, 0); // Align with left edge
        originalXposition = xposition; // Store random x-position
        originalXscale = (rand() % 2) + (level == 1 ? 2 : 1.75); // Store random scale for restoration

        if (widerBlocksActive)
        {
            // Scale to span exactly from Xleft to Xright
            xscale = (Xright - Xleft) / static_cast<float>(texblocks.getSize().x);
            blocksSprite.setPosition(Xleft - 100, yposition); // Align with left wall
        ...
        isMoving = (level == 2 && !widerBlocksActive); // Disable movement if widerBlocksActive
        if (isMoving)
        {
...
struct Players
{
    ...
    {
        static const float FAST_VIEW_SPEED = 900.0f;
...
void generationBlocks(vector<BLOCKS>& blocksList, Players& player, int level)
{
    ...
    int numBlocks = (level == 1) ? blocksNum : blocksNum * level;
    float verticalSpacing = (level == 1) ? 175 : 250;
    ...
    {
        // For level 2, start above the last level 1 block or a fixed offset
        y = (blocksList.empty() ? HEIGHT - 200 : blocksList.back().blocksSprite.getPosition().y) - verticalSpacing;
    }
...
void generationFeatures(Players& player, Text& Score)
{
    ...
    const float spawnInterval = 1.5f; // Spawn attempt every 3 seconds
...
void handleWiderBlocksEffect(vector<BLOCKS>& blocksList, int currentLevel, Players& player)
{
    if (widerBlocksActive && widerBlocksTimer.getElapsedTime().asSeconds() >= 2.0f)
    {
        ...
        // Restore original scale, position, and movement for all blocks
        for (auto& block : blocksList)
        {
            ...
            // Re-enable movement for level 2 blocks
            if (block.level == 2)
            {
                block.isMoving = true;
            ...
            {
                block.isMoving = false; // Ensure level 1 blocks remain static
            }
        }
    }
}
...
void initializeObject()
{
    ...
    background.setScale(static_cast<float>(1920) / tex_background.getSize().x,
        static_cast<float>(1080) / tex_background.getSize().y);

    background2.setTexture(tex_background2);
    background2.setScale(static_cast<float>(1920) / tex_background2.getSize().x,
    ...
    levelBarrier.setScale(5.5f, 1.5f);
...
void reset(Players& player, vector<BLOCKS>& blocksList, Text& Score, bool& viewpaused, bool& win, Clock& rotation, Clock& viewtimer, Clock& Timer, RectangleShape& OclockFill)
{
    ...
    player.sprite.setPosition(960, 750); // Start on ground or first block
...
{
    // Set barrier position if not already set
    if (!barrierSpawned)
    {
        // Place barrier above the last level 1 block
        for (auto it = blocksList.rbegin(); it != blocksList.rend(); ++it)
        {
            if (it->level == 1)
            {
                levelBarrier.setPosition(100, it->blocksSprite.getPosition().y - 300);
                barrierSpawned = true;
                break;
            }
        }
    ...
    // Transition to level 2 based on player's y-position
    if (barrierSpawned && player.sprite.getPosition().y < levelBarrier.getPosition().y - 200 && !doneLevel1)
    {
        currentLevel = 2;
        doneLevel1 = true;
        transitionClock.restart();
        ...
        if (player.sprite.getGlobalBounds().intersects(block.blocksSprite.getGlobalBounds()))
        {
            if (player.velocity_y > 0 && (player.sprite.getPosition().y + player.sprite.getGlobalBounds().height - 50 <= block.blocksSprite.getPosition().y))
            {
                ...
                if (!block.isIntersected)
                {
                    ...
                    // Score based on the block's level
                    if (block.level == 1)
                    {
                        score += 10;
                    }
                ...
                if (lastBlockIndex != -2 && currentBlockIndex != lastBlockIndex && lastBlockIndex < static_cast<int>(blocksList.size()))
                {
                    ...
                    for (int j = startIndex + 1; j < endIndex; j++)
                    {
                        if (!blocksList[j].isIntersected)
                        {
                            ...
                            // Score based on skipped block's level
                            if (blocksList[j].level == 1)
                            {
                                score += 20;
                            }
        ...
        if (feature.active && player.sprite.getGlobalBounds().intersects(feature.sprite.getGlobalBounds()))
        {
            ...
            else if (feature.type == "widerBlocks")
            {
                ...
                for (auto& block : blocksList)
                {
                    ...
                    block.blocksSprite.setPosition(Xleft - 150, block.blocksSprite.getPosition().y); // Align with Xleft
...
int main()
{
    ...
    if (start_game)
    {
        ...
        // Generate blocks for both levels
        generationBlocks(blocksList, player, 1);
        generationBlocks(blocksList, player, 2);
        ...
        while (window.isOpen())
        {
            ...
            if (!viewpaused && player.sprite.getPosition().y < HEIGHT / 7)
            {
                view_movement(deltatime);
                adjustViewAspectRatio(view, window, 1920.0f / 1080.0f);
                generationFeatures(player, Score);
                window.setView(view);
            }
            ...
            // Collision with blocks & level transition
            ...
            window.clear();
            draw(player, blocksList, Score, font, timerText, text_skipped, deltatime, rotation, Timer, OclockFill, showSuperJump);
            window.display();
        }
    }
    return 0;
